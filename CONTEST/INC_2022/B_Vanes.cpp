...
bool palindrome(string str){
    for(ll i = 0, j = str.length() - 1; i <= j; i++,j--){
...
int main(){
    ll n; cin >> n;
    bool possible = true;
    ll tm[n+999], stock[n+999], orders[n+999];
    for(ll i = 1; i <= n-1; i++) cin >> tm[i];
    for(ll i = 1; i <= n; i++) cin >> stock[i];
    for(ll i = 1; i <= n; i++) cin >> orders[i];
    ll sum = 0;

    for(ll i = n; i >= 2; i--){
        ll res = orders[i] - stock[i];
    ...
    if((orders[1] > stock[1])) possible = false;
