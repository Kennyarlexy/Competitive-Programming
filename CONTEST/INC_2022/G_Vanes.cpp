    ...
    void dijkstra(ll start) {
        ...
        while (!mypq.empty()) {
            ...
            for (auto& edge : adjE[current]) {
                ...
                ll newMin = min(dist[nextNode], prevMin + weight);
...
int main() {
    ...
    for(ll i = 1; i <= n; i++){
        char a,b; ll c,d; cin >> a >> c >> b >> d;
        ll max_p_temp = max_p; ll max_p_temp1 = max_p;
        ll min_p_temp = min_p; ll min_p_temp1 = min_p;
        ll temp1 = p, temp = p;
        ...
        if(b == '+'){
                ...
                min_p_temp1 *= d;
                max_p_temp1 *= d;
                if(d < 0 && min_p < 0) temp1 = min_p * d;
                else temp1 = max_p * d;
        }

        min_p = min(min_p_temp1, min_p_temp);
        max_p = max(max_p_temp, max_p_temp1);
        p = max(temp, temp1);
