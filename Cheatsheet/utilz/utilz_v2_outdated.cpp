...
namespace custom_ds {
    ...
    struct RUPQ {
        FenwickTree<T> ft;
        //create RUPQ with size m containing all zeroes
        RUPQ(int m) : ft(m) {}
        //create RUPQ with size m containing equal value val
        RUPQ(int m, int val) : ft(m) {r_update(1, m, val);}
        void r_update(int i, int j, T d) {
            ft.update(i, d);
            ft.update(j+1, -d);
        }
        T rsq(int i) {return ft.rsq(i);}
    };
